    ...
    if (res == XPUM_OK) {
        ...
        switch (res) {
            ...
            case XPUM_UPDATE_FIRMWARE_UNSUPPORTED_AMC:
                response->set_errormsg("Can't find the AMC device. AMC firmware update just works for Intel Data Center GPU (AMC firmware version is 3.6.3 or later) on Intel M50CYP server (BMC firmware version is 2.82 or later).");
                break;
            default:
