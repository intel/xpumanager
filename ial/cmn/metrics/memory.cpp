	...
	QueryMetric{// NOLINT(readability-identifier-naming)
				...
				.getter = [](devInfo &d, MetricValue &out, const MetricCache &) -> ze_result_t {
					...
					if (r == ZE_RESULT_SUCCESS) {
						out = xpum::compat::format("{}", static_cast<double>(size) / (1024.0 * 1024.0));
					}
					return r;
				}};
...
constexpr auto used =
	QueryMetric{// NOLINT(readability-identifier-naming)
				...
				.getter = [](devInfo &d, MetricValue &out, const MetricCache &) -> ze_result_t {
					...
					if (r == ZE_RESULT_SUCCESS) {
						out = xpum::compat::format("{}", static_cast<double>(usedBytes) / (1024.0 * 1024.0));
					}
					return r;
				}};

constexpr auto FREE = QueryMetric{
	...
	.getter = [](devInfo &d, MetricValue &out, const MetricCache &) -> ze_result_t {
		...
		out = xpum::compat::format("{}", static_cast<double>(freeBytes > 0 ? freeBytes : 0) / (1024.0 * 1024.0));
...
constexpr auto READ_BANDWIDTH =
	QueryMetric{// NOLINT(readability-identifier-naming)
				...
				.getter = [](devInfo & /*d*/, MetricValue &out, const MetricCache &cache) -> ze_result_t {
					...
					out = xpum::compat::format("{}", 1000000.0 * rd / dt / 1024.0);
					return ZE_RESULT_SUCCESS;
				}};
...
constexpr auto WRITE_BANDWIDTH =
	QueryMetric{// NOLINT(readability-identifier-naming)
				...
				.getter = [](devInfo & /*d*/, MetricValue &out, const MetricCache &cache) -> ze_result_t {
					...
					out = xpum::compat::format("{}", 1000000.0 * wr / dt / 1024.0);
					return ZE_RESULT_SUCCESS;
				}};
