...
#ifndef XPUM_STRUCTS_H
#define XPUM_STRUCTS_H
...
{
	XPUM_DEVICE_FUNCTION_TYPE_VIRTUAL = 0,
	XPUM_DEVICE_FUNCTION_TYPE_PHYSICAL = 1,
	XPUM_DEVICE_FUNCTION_TYPE_UNKNOWN = 0x7fffffff,
 ...
 * XPUM version info
 */
typedef struct XpumVersionInfo
{
 ...
 * Device basic info
 */
typedef struct XpumDeviceBasicInfo
{
...
typedef enum xpum_device_property_name_enum
{
	...
	XPUM_DEVICE_PROPERTY_MAX_COMMAND_QUEUE_PRIORITY =
		28,									   ///< Maximum priority for command queues. Higher value is higher priority
...
/**************************************************************************/
/**
 * Definitions for device control and telemetry
 */
/**************************************************************************/
 ...
 */
typedef enum xpum_standby_mode_enum
{
	XPUM_STANDBY_MODE_DEFAULT = 0, ///< Default mode
	XPUM_STANDBY_MODE_NEVER = 1	   ///< Device never goes into standby
} xpum_standby_mode_t;

/**
 * @brief ECC states
 */
typedef enum xpum_ecc_state_enum
{
	XPUM_ECC_STATE_UNAVAILABLE = 0, ///< ECC is not available
	XPUM_ECC_STATE_ENABLED = 1,		///< ECC is enabled
	XPUM_ECC_STATE_DISABLED = 2		///< ECC is disabled
} xpum_ecc_state_t;

/**
 * @brief Frequency domains
 */
typedef enum xpum_freq_domain_enum
{
	XPUM_FREQ_DOMAIN_GPU = 0,	///< GPU core frequency
	XPUM_FREQ_DOMAIN_MEMORY = 1 ///< Memory frequency
} xpum_freq_domain_t;

/**
 * @brief Frequency throttle reason flags (bitfield)
 */
typedef enum xpum_freq_throttle_reason_enum
{
	XPUM_FREQ_THROTTLE_REASON_NONE = 0,
	XPUM_FREQ_THROTTLE_REASON_THERMAL_LIMIT = (1 << 0),
	XPUM_FREQ_THROTTLE_REASON_CURRENT_LIMIT = (1 << 1),
	XPUM_FREQ_THROTTLE_REASON_POWER_LIMIT = (1 << 2),
	XPUM_FREQ_THROTTLE_REASON_THERMAL_EXCURSION = (1 << 3),
	XPUM_FREQ_THROTTLE_REASON_BURST_POWER_CAP = (1 << 4),
	XPUM_FREQ_THROTTLE_REASON_AVE_POWER_CAP = (1 << 5)
} xpum_freq_throttle_reason_flags_t;

/**
 * @brief RAS error categories
 */
typedef enum xpum_ras_error_cat_enum
{
	XPUM_RAS_ERROR_CAT_RESET = 0,		///< Errors reset after device reset
	XPUM_RAS_ERROR_CAT_PROGRAMMING = 1, ///< Errors reset after driver reload
	XPUM_RAS_ERROR_CAT_DRIVER = 2,		///< Errors reset after OS reboot
	XPUM_RAS_ERROR_CAT_COMPUTE = 3,		///< Errors reset after device reset
	XPUM_RAS_ERROR_CAT_NON_COMPUTE = 4, ///< Errors reset after device reset
	XPUM_RAS_ERROR_CAT_CACHE = 5,		///< Errors reset after device reset
	XPUM_RAS_ERROR_CAT_DISPLAY = 6		///< Errors reset after device reset
} xpum_ras_error_cat_t;

/**
 * @brief RAS error types
 */
typedef enum xpum_ras_error_type_enum
{
	XPUM_RAS_ERROR_TYPE_CORRECTABLE = 0,  ///< Correctable errors
	XPUM_RAS_ERROR_TYPE_UNCORRECTABLE = 1 ///< Uncorrectable errors
} xpum_ras_error_type_t;
...
{
	XPUM_AGENT_CONFIG_SAMPLE_INTERVAL = 0 ///< Agent sample interval, in milliseconds, options are [100, 200, 500,
										  ///< 1000], default is 1000. Value type in xpumSetAgentConfig is int64_t
} xpum_agent_config_t;
...
typedef enum xpum_stats_type_enum
{
	...
	XPUM_STATS_MEMORY_UTILIZATION = 9,		 ///< Memory Utilization. Percent utilization is calculated by the equation:
											 ///< physical size - free size / physical size. Unit %
	...
	XPUM_STATS_FREQUENCY_THROTTLE_REASON_GPU =
		38, ///< Frequency Throttle reason, refer to the document of zes_freq_throttle_reason_flags_t
...
typedef struct xpum_device_fabric_throughput_stats_t
{
	...
	uint32_t remoteDeviceId;			///< remote device id
	uint32_t remoteDeviceTileId;		///< remote tile id
...
#define XPUM_MAX_POWER_LIMITS 8

typedef struct xpum_power_domain_ext_t
{
	int32_t powerDomain;
	xpum_power_limit_ext_t plExt[XPUM_MAX_POWER_LIMITS];
	int32_t plExtCount;
...
{
	xpum_power_sustained_limit_t sustainedLimit;
} xpum_power_limits_t;
...
typedef struct xpum_scheduler_data_t
{
	...
	xpum_engine_type_flags_t engineTypes;
	xpum_scheduler_mode_t supportedModes;
	uint64_t val1;
	uint64_t val2;
...
typedef struct XpumXelinkTopoInfo
{
	XpumXelinkUnit localDevice;
	XpumXelinkUnit remoteDevice;
...
typedef struct xpum_policy_action_t
{
	...
	double throttleDeviceFrequencyMin;
	double throttleDeviceFrequencyMax;
...
typedef struct xpum_policy_t
{
	...
	bool isDeletePolicy; // Only for set policy api, ignored by get policy api. If true, then delete this policy in set
						 // policy api.
} xpum_policy_t;
...
typedef struct xpum_vgpu_config_t
{
	...
	uint64_t lmemPerVf; ///< Local memory per vGPU (in bytes), if set to 0 then apply the configuration file to allocate
						///< local memory for vGPUs
} xpum_vgpu_config_t;
...
typedef struct XpumPrecheckOptions
{
	bool onlyGPU;		   ///< Check GPU-related error and ignore CPU error or not
	const char *sinceTime; ///< Start time for log scanning. Only works when the logSource is JOURNALCTL. Scanning would
						   ///< start from the latest boot if it is NULL. The generic format is "YYYY-MM-DD HH:MM:SS".
						   ///< Alternatively the strings "yesterday", "today" are also understood. Relative times also
						   ///< may be specified, prefixed with "-" referring to times before the current time.
} XpumPrecheckOptions;
