...
ze_result_t device::smDevInit(zes_driver_handle_t zesDri, zes_device_handle_t zesDev)
{
	...
	// If the call fails (e.g. some UMD versions return ZE_RESULT_ERROR_SURVIVABILITY_MODE_DETECTED
	// for healthy devices), igpu stays false — a conservative default that keeps all normal
	// device paths working rather than aborting initialization.
	zes_device_properties_t zesProps = {};
	zesProps.stype = ZES_STRUCTURE_TYPE_DEVICE_PROPERTIES;
	if (zesGetDevProps(zesDev, &zesProps) == ZE_RESULT_SUCCESS) {
