...
std::vector<DumpTypeOption> dumpTypeOptions{
    ...
    {XPUM_DUMP_EU_STALL, DUMP_OPTION_STATS, XPUM_STATS_EU_STALL, XPUM_ENGINE_TYPE_UNKNOWN, "XPUM_STATS_EU_STALL", "GPU EU Array Stall (%)", "the normalized sum of all cycles on all EUs during which the EUs were stalled. Per tile or device. Device-level is the average value of tiles for multi-tiles.\n    At least one thread is loaded, but the EU is stalled. Per tile."},
