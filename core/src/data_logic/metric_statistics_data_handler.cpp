...
void MetricStatisticsDataHandler::updateStatistics(std::shared_ptr<SharedData>& p_data) {
    ...
    while (iter != p_data->getData().end()) {
        for (uint64_t session = 0; session < MAX_STATISTICS_SESSION_NUM; session++) {
...
MeasurementData MetricStatisticsDataHandler::getLatestStatistics(std::string& device_id, uint64_t session_id) noexcept {
    std::unique_lock<std::mutex> lock(this->mutex);
    if (p_latestData == nullptr) {
        return MeasurementData();
    }
