...
powershell -Command "(Get-Content winxpum\cli\resource.h) -replace 'VER_COMMIT_VERSION VER_VERSION_MAJORMINORPATCH_STR', 'VER_COMMIT_VERSION_TEMP \"%COMMIT%\"' | Out-File  -encoding ASCII winxpum\cli\resource.h"
powershell -Command "(Get-Content winxpum\core\resource.h) -replace 'VER_COMMIT_VERSION VER_VERSION_MAJORMINORPATCH_STR', 'VER_COMMIT_VERSION_TEMP \"%COMMIT%\"' | Out-File  -encoding ASCII winxpum\core\resource.h"
...
powershell Compress-Archive xpu-smi.exe,xpum.dll,xpum.lib,xpum_api.h,xpum_structs.h xpu-smi-%version%-%TIMESTAMP%.%COMMIT_SHORT%_win.zip
pushd ..\..\..\install\tools\signfile
SignFile.exe -u %SIGN_USER% -p %SIGN_PASS% -conf SignFileWin.config.xml -s cl -cf %WORK_DIR%\winxpum\Release\xpu-smi-%version%-%TIMESTAMP%.%COMMIT_SHORT%_win.zip.sig %WORK_DIR%\winxpum\Release\xpu-smi-%version%-%TIMESTAMP%.%COMMIT_SHORT%_win.zip
popd