...
static int card_detect(nrv_card *card) {
    ...
    if (int ee = bsmc_hal->validate_res(res, CARD_GET_INFO_RES_MIN_SIZE)) {
        return NRV_IPMI_ERROR;
    }
...
static int init_card_list() {
    ...
    card.ipmi_address.bus = 0;
    card.ipmi_address.slot = 0;
    card.ipmi_address.i2c_addr = CARD_FIRST_I2C_ADDR_OLD;
    ...
    for (auto i2c_addr : slaveAddrs) {
        for (uint8_t slot = 0; slot < slot_count; slot++) {
            card.ipmi_address.bus = 0;
            card.ipmi_address.slot = slot;
            card.ipmi_address.i2c_addr = i2c_addr;
