...
│   ├── validators.py       # Return-code, plaintext, JSON, combined-output validators
│   ├── executor.py         # Subprocess wrapper (captures stdout+stderr, enforces timeout)
│   ├── models.py           # TestResult dataclass
│   ├── resolver.py         # Parameterised-test expansion
...
| `-b / --binary` | Path to the `xpu-smi` binary under test |
...
- **Workflow tests** — multi-step sequences with background steps, `depends_on`, `cleanup_step`, `run_on_failure`, `condition`
- **Wrapper** — wrap a command with another tool (e.g. `perf stat`, `timeout`)
- **Script** — arbitrary shell script with `{command}` substitution
- **Hooks** — structured `before_test` / `after_test` / `on_failure` shell snippets
- **OS and platform filters** — `os: Linux`, `platform: [BMG]`
