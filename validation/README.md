...
│   ├── resolver.py         # Dependency resolution / parameterized-test ordering
│   ├── color.py            # ANSI color helpers (auto-disabled when not a TTY)
│   └── utils.py            # YAML loading, device discovery helpers
└── <suites>/               # End-to-end test suites (one per subcommand + global)
    ├── global.yaml
    ├── discovery.yaml
    ├── topology.yaml
    ├── health.yaml
...
Throughout this document `<suites>` stands for the directory that holds the YAML
suites — the single subdirectory of `validation/` containing `*.yaml` files.
Substitute its actual name (`ls validation/*/*.yaml | head -1`) in the commands
below, or let `run_e2e.sh` detect it for you.  Some distributions of this
repository ship a different suite set, so the name is deliberately not baked
into the tooling.
...
Usage: run_e2e.sh [options]
  ...
  -s <dir>    Directory holding the YAML test suites
              Default: auto-detected (the one subdirectory of validation/
              that contains *.yaml suites)
              Override with: XPU_SMI_SUITES=/path/to/suites ./run_e2e.sh
...
# Explicit suite directory (needed only when several are present)
bash run_e2e.sh -s "$(ls -d validation/*/ | head -1)"
...
# Single suite
python run_tests.py -c <suites>/discovery.yaml \
                    -b ../builddir/ial/cli/xpu-smi

# All e2e suites, parallel, with issue files
python run_tests.py -c <suites>/ -r \
                    -b ../builddir/ial/cli/xpu-smi \
                    -j 4 \
...
python run_tests.py -c <suites>/dump_metrics_matrix.yaml \
                    -b ../builddir/ial/cli/xpu-smi \
...
python run_tests.py -c <suites>/ -r --list-tests
```
...
`--tags` and `-k/--test-name` are applied *after* parameterized expansion, so an
...
against the detected host OS.  Both gates are honored by command tests and
...
- **Parameterized tests** — explicit list, numeric range, or auto-discovered device IDs
...
`run_tests.py` produces three machine-readable artifacts:
...
```bash
python3 run_coverage.py -c <suites>/ -j 4
```
...
`-c <suites>/` explicitly.  Exit codes match `run_e2e.sh`: `0` pass, `1` test
...
python run_tests.py -c <suites>/ -r --tags sanity json
# everything except tests that mutate device state or need root
python run_tests.py -c <suites>/ -r --exclude-tags intrusive sudo
```
...
1. Create `<suites>/<subcommand>.yaml` using the template below.
