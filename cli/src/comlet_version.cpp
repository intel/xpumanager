...
void ComletVersion::getTableResult(std::ostream &out) {
    auto res = run();
    out << "CLI:" << std::endl;
    out << "    Version: " << (res->contains("cli_version") ? (*res)["cli_version"] : "") << std::endl;
    out << "    Build ID: " << (res->contains("cli_version_git") ? (*res)["cli_version_git"] : "") << std::endl;
    out << std::endl;
    out << "Service:" << std::endl;
    out << "    Version: " << (res->contains("xpum_version") ? (*res)["xpum_version"] : "") << std::endl;
    out << "    Build ID: " << (res->contains("xpum_version_git") ? (*res)["xpum_version_git"] : "") << std::endl;
    out << "    Level Zero Version: " << (res->contains("level_zero_version") ? (*res)["level_zero_version"] : "") << std::endl;
}
